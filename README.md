...
**V0.0.1 source preview** | Rust native runtime + native editor | Windows first
...
V0.0.1 是首个公开源码预览版，适合源码阅读、架构评估和早期功能试用。目前仍处于快速开发阶段，不建议直接用于生产项目。
...
AI First Game Engine V0.0.1 is an experimental, Windows-first source preview built around a Rust native runtime and editor. It explores schema-driven game authoring, auditable AI project mutation, RuntimePackage-based execution, native editor workflows, WGPU rendering, and runtime AUI. This engine-only release excludes all sample games, temporary tower-defense content, internal construction evidence, legacy prototypes, binaries, and archives. The project is currently `UNLICENSED` and not production-ready.
