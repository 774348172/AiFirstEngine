...
pub use project_authoring_execution::{
    EngineBuiltInFontPackError, EngineBuiltInFontPackManifest, ENGINE_BUILT_IN_FONT_PACK_ID,
    ENGINE_BUILT_IN_FONT_PACK_MANIFEST_SCHEMA_VERSION,
};
...
impl EngineBuiltInFontPack {
    ...
    pub fn produce(
        ...
        let mut source_bytes_by_id = BTreeMap::new();
        source_bytes_by_id.insert(FONT_SOURCE_ID.to_string(), source_bytes);
        let mut output = ProjectFontCookModule::cook(ProjectFontCookRequest {
            assets: built_in_assets(profile.clone(), &source_sha256),
            source_bytes: source_bytes_by_id,
    ...
    pub fn load_embedded() -> Result<RuntimePackageSourceFontBundle, EngineBuiltInFontPackError> {
        project_authoring_execution::load_engine_builtin_font_pack()
