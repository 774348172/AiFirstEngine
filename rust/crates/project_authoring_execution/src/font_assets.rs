...
pub struct ProjectFontAssetGraphDocument {
    pub schema_version: String,
    pub built_from_database_version: u64,
    pub nodes: Vec<ProjectFontAssetGraphNode>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct ProjectFontAssetGraphNode {
...
pub fn validate_font_face_source_bytes(
    source_label: &str,
    bytes: &[u8],
    asset: &FontFaceAsset,
) -> Result<ValidatedFontFaceSource, FontDiagnostic> {
    ...
    validate_declared_metadata(asset, &face)?;
    Ok(ValidatedFontFaceSource {
        canonical_path: PathBuf::from(source_label),
        source_sha256: actual_hash,
        face_count,
    })
}
...
impl ProjectFontAssetSet {
    ...
    pub fn asset_graph(&self, database_version: u64) -> ProjectFontAssetGraphDocument {
        let mut nodes = Vec::new();
        nodes.extend(self.faces.iter().map(|asset| ProjectFontAssetGraphNode {
        ...
        nodes.extend(self.stacks.iter().map(|asset| ProjectFontAssetGraphNode {
            asset_guid: asset.asset_id.clone(),
            asset_id: asset.asset_id.clone(),
            direct_dependencies: asset.families.clone(),
            source_paths: Vec::new(),
        }));
        nodes.extend(self.profiles.iter().map(|asset| ProjectFontAssetGraphNode {
            asset_guid: asset.asset_id.clone(),
            asset_id: asset.asset_id.clone(),
