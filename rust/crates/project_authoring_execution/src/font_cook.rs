use crate::{
    resolve_font_family_face, validate_font_face_source_bytes, validate_font_stack,
    FontAtlasProfileAsset, FontDiagnostic, FontDiagnosticSeverity, FontDiagnosticStage,
    FontFaceAsset, FontFamilyAsset, FontRasterPolicy, FontStackAsset, FontStyle,
    ProjectFontAssetSet, PROJECT_FONT_RECIPE_VERSION,
};
use crate::{
...
impl ProjectFontCookModule {
    ...
    fn cook_timed(
        ...
        for face in faces.values() {
            ...
            let validated = validate_font_face_source_bytes(&face.source.asset_ref, bytes, face)
                .map_err(single_failure)?;
            source_hashes.insert(face.source.asset_ref.clone(), validated.source_sha256);
            source_bytes.insert(face.source.asset_ref.clone(), bytes.clone());
        }
    ...
    pub(crate) fn cook_for_runtime_package_from_source_view(
        source: &crate::game_project_compiler::CompilerSourceView,
    ...
    > {
        ...
        let prepared = prepare_project_font_cook(source, documents, profile_id)?;
        let recipe_duration_ms = elapsed_ms(recipe_started);
        let Some(mut prepared) = prepared else {
            return Err(failure(
                "FontDefaultUiStackInvalid",
                "No project FontAtlasProfile was available for project font cooking.",
            ));
        };
...
fn prepare_project_font_cook(
    ...
    let loaded = load_project_request(source, documents, profile_id)?;
    ...
    for face in &request.assets.faces {
        let source_bytes = request
            ...
            .get(&face.source.asset_ref)
            .ok_or_else(|| failure("FontSourceAssetRefMissing", &face.source.asset_ref))?;
        let validated = validate_font_face_source_bytes(&face.source.asset_ref, source_bytes, face)
            .map_err(single_failure)?;
        source_hashes.insert(face.source.asset_ref.clone(), validated.source_sha256);
    }
    let collected = collect_required_text(&request, &profile).map_err(single_failure)?;
...
pub(crate) fn source_has_project_font_profile(
    source: &crate::game_project_compiler::CompilerSourceView,
) -> bool {
    source
        .paths()
        .filter(|path| path.starts_with("Assets/") && path.ends_with(".json"))
        .filter_map(|path| source.bytes(path))
...
fn load_project_request(
    ...
    let asset_paths = source
        .paths()
        .filter(|path| path.starts_with("Assets/") && path.ends_with(".json"))
        .map(str::to_string)
        .collect::<Vec<_>>();
    for path in &asset_paths {
        let bytes = source.bytes(path).expect("SourceView path must own bytes");
    ...
    for path in source
        .paths()
        .filter(|path| path.starts_with("Localization/") && path.ends_with(".json"))
    {
    ...
    for path in source
        .paths()
        .filter(|path| path.starts_with("Assets/") && path.ends_with(".meta.json"))
    {
        let Some(bytes) = source.bytes(path) else {
            continue;
        };
        let Ok(value) = serde_json::from_slice::<serde_json::Value>(bytes) else {
            continue;
        };
        if value.get("assetType").and_then(serde_json::Value::as_str) != Some("fontSource") {
            continue;
        }
        let Some(asset_id) = value.get("assetId").and_then(serde_json::Value::as_str) else {
            continue;
        };
        let Some(source_path) = value.get("sourcePath").and_then(serde_json::Value::as_str) else {
            continue;
        };
        let bytes = source.bytes(source_path).ok_or_else(|| {
            failure(
                "FontSourceAssetRefMissing",
                &format!("{asset_id} references missing leased source {source_path}"),
            )
        })?;
        source_bytes.insert(asset_id.to_string(), bytes.to_vec());
    }
...
mod tests {
    ...
    fn fixture_bytes() -> Vec<u8> {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("AifeNotoSansSCQualification-Regular.ttf");
        std::fs::read(path).expect("qualification fixture font")
    }

    #[test]
    fn compiler_prepare_reuses_font_cook_across_unrelated_rust_source_changes() {
        let mut request = request("A");
        request.assets.profiles[0].glyph_set.literals = vec!["A".into()];
        let mut source = crate::CompilerSourceView {
            files: BTreeMap::new(),
        };
        source.files.insert(
            "project.aife.json".into(),
            br#"{"projectId":"font-cache","projectName":"font-cache"}"#.to_vec(),
        );
        for (name, value) in [
            (
                "face",
                serde_json::to_value(&request.assets.faces[0]).unwrap(),
            ),
            (
                "family",
                serde_json::to_value(&request.assets.families[0]).unwrap(),
            ),
            (
                "stack",
                serde_json::to_value(&request.assets.stacks[0]).unwrap(),
            ),
            (
                "profile",
                serde_json::to_value(&request.assets.profiles[0]).unwrap(),
            ),
        ] {
            source.files.insert(
        ...
        source.files.insert("Assets/font.meta.json".into(), serde_json::to_vec(&serde_json::json!({"assetType":"fontSource","assetId":SOURCE_ID,"sourcePath":"Assets/font.ttf"})).unwrap());
        source
            .files
            .insert("Assets/font.ttf".into(), fixture_bytes());
        let root = std::env::temp_dir().join(format!(
            "aife-font-incremental-{}-{}",
            std::process::id(),
        ...
        let (_, _, first) = crate::neutral_assembler::assemble(&source, Some(&root)).unwrap();
        assert_ne!(
            first[0].cache_status,
            ProjectAssemblyArtifactCacheStatus::Hit
        );
        source.files.insert(
            "RuntimeModule/src/unrelated.rs".into(),
            b"// modified Rust\n".to_vec(),
        );
        let (_, _, second) = crate::neutral_assembler::assemble(&source, Some(&root)).unwrap();
        assert_eq!(
            second[0].cache_status,
            ProjectAssemblyArtifactCacheStatus::Hit
        );
        assert_eq!(first[0].recipe_key, second[0].recipe_key);
        assert!(second[0]
            .substages
            .iter()
            .any(|stage| stage.stage_id == "raster_bitmap" && stage.skipped));
    ...
    fn request(aui_text: &str) -> ProjectFontCookRequest {
        ...
        ProjectFontCookRequest {
            ...
            source_bytes: BTreeMap::from([(SOURCE_ID.to_string(), fixture_bytes())]),
    ...
    #[test]
    fn asymmetric_glyph_orientation_fresh_cook_and_sealed_pack() {
        // Compare occupied silhouettes, normalized to their own ink bounds so padding,
        // hinting and glyph size cannot disguise a vertical reflection.
        fn normalized(mask: &[bool], width: usize) -> Vec<bool> {
            let ink: Vec<_> = mask
                .iter()
                .enumerate()
                .filter(|(_, v)| **v)
            ...
            (0..1024)
                .map(|i| {
                    let x = x0 + (i % 32 * 2 + 1) * (x1 - x0 + 1) / 64;
                    let y = y0 + (i / 32 * 2 + 1) * (y1 - y0 + 1) / 64;
                    mask[y * width + x]
                })
                .collect()
        }
        fn distances(a: &[bool], b: &[bool]) -> (usize, usize) {
            let upright = (0..1024).filter(|&i| a[i] != b[i]).count();
            let reflected = (0..1024)
                .filter(|&i| a[i] != b[(31 - i / 32) * 32 + i % 32])
                .count();
            (upright, reflected)
        }
        fn median(pixel: &[u8]) -> bool {
            let mut c = [pixel[0], pixel[1], pixel[2]];
            c.sort_unstable();
            c[1] > 128
        }
        fn occupancy(mask: &[bool]) -> (usize, usize) {
            (
                mask[..512].iter().filter(|v| **v).count(),
                mask[512..].iter().filter(|v| **v).count(),
            )
        }
        let mut input = request("Agpy2");
        let source = std::fs::read(std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../resources/runtime/font-packs/aife-default-zh-cn-common-v1/source/NotoSansSC-Regular.ttf"))
            .expect("complete built-in source font");
        input.assets.faces[0].source.source_sha256 = sha256_prefixed(&source);
        input.assets.faces[0].declared.family = "Noto Sans SC".into();
        input.source_bytes.insert(SOURCE_ID.into(), source);
        input.assets.profiles[0].raster.bitmap_pixel_sizes = vec![16, 20, 24, 34];
        input.assets.profiles[0].packing.page_width = 512;
        input.assets.profiles[0].packing.page_height = 512;
        let output = ProjectFontCookModule::cook(input).unwrap();
        let pack = crate::load_engine_builtin_font_pack().unwrap();
        let mut failures = Vec::new();
        for ch in "Agpy2".chars() {
            let msdf = output
                .msdf_variants
                .iter()
                .find(|v| v.codepoint == ch as u32)
                .expect("MSDF glyph");
            let fresh = normalized(
                &msdf.rgba8.chunks_exact(4).map(median).collect::<Vec<_>>(),
                msdf.width as usize,
            );
            for size in [16, 20, 24, 34] {
                let bitmap = output
                    .hinted_variants
                    .iter()
                    .find(|v| v.codepoint == ch as u32 && v.pixel_size == size)
                    .expect("bitmap glyph");
                let reference = normalized(
                    &bitmap.alpha_r8.iter().map(|&v| v > 128).collect::<Vec<_>>(),
                    bitmap.width as usize,
                );
                let (same, flipped) = distances(&reference, &fresh);
                println!("fresh {ch}/{size}: upright={same} reflected={flipped} bitmap_top_bottom={:?} msdf_top_bottom={:?}", occupancy(&reference), occupancy(&fresh));
                if same >= flipped {
                    failures.push(format!("fresh {ch}/{size}: {same}>={flipped}"));
                }
            }
            ...
            for glyph in pack
                ...
                .glyphs
                .iter()
                .filter(|g| g.codepoint == ch as u32)
            {
                let page = &pack.metadata.pages[glyph.page_index as usize];
                let payload = &pack.page_payloads[glyph.page_index as usize];
                let bpp = if glyph.render_mode
                    == engine_runtime::font_bundle::FontBundleRenderMode::BitmapR8
                {
                    1
                } else {
                    4
                };
                let [x, y, w, h] = glyph.pixel_rect;
                let mut mask = Vec::new();
                for row in y..y + h {
                    for col in x..x + w {
                        let i = (row as usize * page.width as usize + col as usize) * bpp;
                        mask.push(if bpp == 1 {
                            payload[i] > 128
                        } else {
                            median(&payload[i..i + 4])
                        });
                    }
                }
                modes.push((
                    glyph.render_mode,
                    glyph.pixel_size,
                    normalized(&mask, w as usize),
                ));
            }
            let bitmap = modes
                .iter()
                .find(|m| {
                    m.0 == engine_runtime::font_bundle::FontBundleRenderMode::BitmapR8 && m.1 == 24
                })
                .expect("sealed bitmap");
            for (mode, size, mask) in &modes {
                let (same, flipped) = distances(&bitmap.2, mask);
                println!("sealed {ch}/{size}/{mode:?}: upright={same} reflected={flipped}");
                if same >= flipped {
                    failures.push(format!("sealed {ch}/{size}/{mode:?}: {same}>={flipped}"));
                }
                if *mode == FontBundleRenderMode::MsdfRgba8 {
                    let (same, flipped) = distances(&fresh, mask);
                    println!("fresh_vs_sealed {ch}: upright={same} reflected={flipped}");
                    if same >= flipped {
                        failures.push(format!("fresh_vs_sealed {ch}: {same}>={flipped}"));
                    }
                }
            }
        }
        assert!(failures.is_empty(), "orientation failures: {failures:#?}");
    }
