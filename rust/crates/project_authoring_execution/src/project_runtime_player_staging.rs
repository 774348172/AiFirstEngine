...
const TRUSTED_ENGINE_DEPENDENCIES: &[&str] = &["project_game_sdk"];
...
#[derive(Debug, Clone)]
pub struct ProjectRuntimePlayerStagingPlan {
...
#[derive(Debug)]
pub struct ProjectRuntimePlayerStagingError {
    pub code: String,
    pub message: String,
}
...
pub struct ProjectRuntimeProductionStaging;
...
pub(crate) fn plan_project_runtime_player_staging(
    ...
    if !dependencies.contains_key("project_game_sdk") {
        return Err(ProjectRuntimePlayerStagingError::new(
            "project_runtime.player_artifact_staging_manifest_policy_rejected",
            "RuntimeModule requires project_game_sdk as its only Engine-facing dependency.",
        ));
    }
    ...
    match value {
        ...
        toml::Value::Table(table) => {
            ...
            if let Some(path) = table.get("path").and_then(toml::Value::as_str) {
                ...
                if name == "project_game_sdk" && declared.is_relative() {
...
[dependencies]
project_game_sdk = {{ path = "{engine_runtime}" }}
...
"#,
        ...
        let engine_identity = plan
            ...
            .iter()
            .find(|dependency| dependency.name == "project_game_sdk")
            .unwrap();
        assert_eq!(
            engine_identity.source_identity,
            sdk.join("crates/project_game_sdk")
                .canonicalize()
                .unwrap()
                .display()
                .to_string()
        ...
        let serde_identity = plan
            ...
            .unwrap();
        let locked = parse_trusted_lock(&source_lock_before).unwrap();
        let requirement = VersionReq::parse("1").unwrap();
        assert_eq!(
            serde_identity.resolved_version,
            resolve_locked_package(&locked, "serde", &requirement)
                .unwrap()
                .version
                .to_string()
        );
        let serde_json_identity = plan
            ...
            .unwrap();
        assert_eq!(
            serde_json_identity.resolved_version,
            resolve_locked_package(&locked, "serde_json", &requirement)
                .unwrap()
                .version
                .to_string()
        );
        ...
        .unwrap();
        assert_eq!(
            normalized["dependencies"]["project_game_sdk"]["path"].as_str(),
    ...
    fn project_runtime_player_production_staging_resolves_relative_sdk_from_arbitrary_build_root() {
        ...
        assert!(source_manifest.contains("../../../rust/crates/project_game_sdk"));
        ...
        for dependency in ["project_game_sdk"] {
